...
export class ScopeGuard {
    ...
    setSuspended(v) { this.suspended = v; return this.suspended; }
    state() {
        return { suspended: this.suspended, enforceScope: this.opts.enforceScope };
    }
    /** 主入口：同步判定（全部为本地文件读 + 内存匹配，2s 缓存节流）。永不 throw。 */
    decide(toolName, args, agentId) {
        try {
            if (OWN_TOOLS.has(toolName))
                return null; // 本插件工具永放行（L2/L3 门自管）
            if (META_ALLOW.has(toolName))
                return null; // bash/run_code：放行，观测面已记录
            if (READ_ALWAYS.has(toolName))
                return null;
            if (READ_PREFIXES.some(p => toolName.startsWith(p)))
                return null;
            const pathFields = WRITE_PATH_TOOLS[toolName];
            if (pathFields == null)
                return null; // 未知/中性工具：放行（tool-calls.jsonl 已观测）
            const paths = extractPaths(args, pathFields);
            if (paths.length === 0)
                return null; // 无法提取路径：不误判
            const doing = this.doingTasks();
            if (doing.length === 0)
                return null; // 无进行中任务：无判定上下文，放行
            ...
            for (const t of withScope) {
                for (const p of paths) {
                    if (scopePathHit(p, t.scope, t.project) !== '')
                        return null; // 任一 doing 任务 scope 命中 → 放行
                }
            }
            ...
            if (!enforcing)
                return null; // 挂起/仅告警模式：放行（事件已留痕）
            return { kind: 'deny', reason };
        }
        catch (e) {
            try {
                this.opts.log?.('guard 内部错误已放行: ' + (e instanceof Error ? e.message : String(e)));
            }
            catch { /* 静默 */ }
            return null;
        }
    ...
    doingTasks() {
        const now = Date.now();
        const projects = new Set([join(this.opts.stateDir, 'default')]);
        try {
    ...
    throttledEvent(t, kind, minGapMs, payload) {
        try {
...
/** 参数中提取路径字段（仅字符串；防原型链）。 */
function extractPaths(args, fields) {
