...
import { BoardError, applyOps, lintBoard, newBoard, stableStringify, topoOrder, } from './schema.js';
...
export class PlanStore {
    ...
    opts;
    broadcast;
    constructor(projectDir, opts, broadcast) {
        ...
        if (errs.length > 0)
            throw new BoardError('BOARD_INVALID', 'plan.board.json 校验失败:\n- ' + errs.join('\n- '));
        this.board = parsed;
    ...
    static ensure(projectDir, opts, init, broadcast) {
        try {
    ...
    static open(projectDir, opts, broadcast) {
        return new PlanStore(projectDir, opts, broadcast);
    }
    addEvent(kind, actor, payload) {
        const ev = { ts: new Date().toISOString(), seq: this.events.length + 1, kind, actor, payload };
        this.events.push(ev);
        try {
        ...
        try {
            this.broadcast?.(this.projectDir, ev);
        }
        catch { /* 订阅者失败不阻断主流程 */ }
    }
    eventsTail(n) {
    ...
    mutate(actor, kind, payload, fn, expectedVersion) {
        ...
        if (errs.length > 0)
            throw new BoardError('BOARD_INVALID', '变更未生效（校验门拦截）:\n- ' + errs.join('\n- '));
    ...
    mirrorRoadmap() {
        ...
        lines.push('');
        const mark = { done: 'x', doing: '~', blocked: '!', canceled: ' ' };
        ...
        for (const m of mods) {
            ...
            for (const t of b.nodes.filter(n => n.type === 'task' && n.parent === m.id)) {
                ...
                if (t.note != null && t.note !== '')
                    lines.push('  - 备注: ' + t.note);
            }
            lines.push('');
        }
        if (b.driftBlock != null)
            lines.push('> ⛔ 漂移阻断中: ' + b.driftBlock.reason + '（面板处置后解除）');
        if (b.pendingApprovals.length > 0)
            lines.push('> ⏳ 待批 plan_edit: ' + b.pendingApprovals.map(a => a.id).join(', '));
        writeFileSync(join(this.dir, 'ROADMAP.md'), lines.join('\n'));
    }
    /** L4 git 交叉核对（状态流转时调用）。返回 'block'|'alert'|'ok'|'skip'。 */
    driftCheck(taskId, actor) {
        const t = this.board.nodes.find(n => n.id === taskId);
        if (t == null || t.type !== 'task')
            return 'skip';
        const scope = t.scope ?? [];
        if (scope.length === 0 || t.startedAt == null)
            return 'skip';
