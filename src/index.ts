...
export function apply(ctx: HostCtx, config: Partial<Config> = {}): () => void {
  const opts: StoreOpts = { blockOnDrift: config.blockOnDrift ?? true }
  ...
  ctx.inject(['webServer', 'tools'], (hostCtx) => {
    const host = hostCtx as HostCtx
    if (host.webServer == null) throw new Error('dsh-plan-board 需要 webServer 服务（web profile 才提供）')
    if (host.tools == null) throw new Error('dsh-plan-board 需要 tools 服务')
    const log = (msg: string): void => {
      try { host.logger?.info?.('[dsh-plan-board] ' + msg) } catch { /* logger 缺失不致命 */ }
    }
    ...
    cleanups.push(mountRoutes(host.webServer as never, registry))
    registerBoardTools(host.tools as never, stateDir, opts, registry.onEvent, (p, n) => registry.remember(p, n))
    ...
    const evCtx = ctx as unknown as {
      on(name: 'tools/pre-execute', fn: (exec: { name: string; arguments: unknown; agent?: { id?: string } }, next: () => Promise<unknown>) => Promise<unknown>): () => void
    }
    ...
    cleanups.push(() => { try { offPre() } catch { /* 已卸载 */ } })

    if (config.observeTools !== false) {
      const off = ctx.on('tools/result', (exec) => {
        try {
          const toolName = String((exec as unknown as { name?: string }).name ?? '')
          if ((TOOL_NAMES as readonly string[]).includes(toolName)) return
          const agent = String((exec as unknown as { agent?: { id?: string } }).agent?.id ?? 'unknown')
          registry.observeToolCall(toolName, agent)
        } catch { /* 观测失败不阻断工具执行 */ }
      })
      cleanups.push(() => { try { off() } catch { /* 已卸载 */ } })
    }
    // cordis 约定：inject 回调返回函数 = 该注入作用域的 dispose（卸载即净的关键路径）
    return () => {
      for (const c of cleanups) { try { c() } catch { /* 幂等 */ } }
    }
  })
  ...
  return () => {
    if (disposed) return
    disposed = true
    for (const c of cleanups) { try { c() } catch { /* 卸载幂等 */ } }
  }
}
